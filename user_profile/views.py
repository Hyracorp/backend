...
from .serializers import TenantUserProfileSerializer
...
class TenantUserProfileView(APIView):
    ...
    def get(self, request, id=None):
        if id is None:
            return Response(data={'message': 'Id is required to retrieve'}, status=status.HTTP_400_BAD_REQUEST)
        user = request.user
        ...
        if tenant_profile:
            ...
            return Response(data={'message': 'Profile not found, pls update'}, status=status.HTTP_404_NOT_FOUND)
    ...
    def put(self, request, id=None):
        if id is None:
            return Response(data={'message': 'Id is required to update'}, status=status.HTTP_400_BAD_REQUEST)
        request.data['user'] = request.user.id
